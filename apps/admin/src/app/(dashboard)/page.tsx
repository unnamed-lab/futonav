...
import { MapPin, Plus, Upload, Layers, Building, ShieldAlert, List, ArrowRight, Activity, Cpu } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  const categories = [
    ...
    { name: "Gate", count: categoryCount["Gate"] || 0, color: "bg-slate-500", text: "text-slate-600", border: "border-slate-100" },
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-slate-200/80">
        ...
        <div className="flex items-center gap-3 bg-white/85 border border-slate-200/80 rounded-2xl px-4 py-2.5 backdrop-blur-md shadow-xs">
      ...
      {errorMsg && (
        <div className="rounded-2xl border border-red-200 bg-red-50 p-5 flex gap-4 items-start shadow-sm shadow-red-200/20">
          ...
          <div>
            ...
            <p className="text-xs text-red-500 mt-3 font-mono font-semibold">
      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="glass-panel rounded-2xl p-6 flex items-center gap-5 relative group overflow-hidden">
          <div className="absolute top-0 right-0 w-24 h-24 bg-teal-500/[0.02] rounded-full blur-2xl -mr-6 -mt-6 transition-all group-hover:bg-teal-500/[0.05]" />
          <div className="p-4 rounded-xl bg-teal-50 text-teal-600 border border-teal-100/80 shadow-xs">
            <MapPin className="h-7 w-7" />
          </div>
          <div>
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Total Surveyed POIs</span>
            <h2 className="text-3xl font-black text-slate-900 mt-1 leading-none tracking-tight">{totalPois}</h2>
        ...
        <div className="glass-panel rounded-2xl p-6 flex items-center gap-5 relative group overflow-hidden">
          <div className="absolute top-0 right-0 w-24 h-24 bg-indigo-500/[0.02] rounded-full blur-2xl -mr-6 -mt-6 transition-all group-hover:bg-indigo-500/[0.05]" />
          <div className="p-4 rounded-xl bg-indigo-50 text-indigo-600 border border-indigo-100/80 shadow-xs">
            <Layers className="h-7 w-7" />
          </div>
          <div>
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Categories Configured</span>
            <h2 className="text-3xl font-black text-slate-900 mt-1 leading-none tracking-tight">
        ...
        <div className="glass-panel rounded-2xl p-6 flex items-center gap-5 relative group overflow-hidden">
          <div className="absolute top-0 right-0 w-24 h-24 bg-sky-500/[0.02] rounded-full blur-2xl -mr-6 -mt-6 transition-all group-hover:bg-sky-500/[0.05]" />
          <div className="p-4 rounded-xl bg-sky-50 text-sky-600 border border-sky-100/80 shadow-xs">
            <Cpu className="h-7 w-7" />
          </div>
          <div>
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block">Target Datastore</span>
            <h2 className="text-lg font-black text-slate-900 mt-1 leading-none tracking-tight">Supabase Sync</h2>
            <span className="text-[10px] text-slate-500 block mt-2.5 font-mono font-medium">schema: public.pois</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <div className="glass-panel rounded-2xl p-6 lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between pb-3 border-b border-slate-200/80">
            <h2 className="text-lg font-black text-slate-900">Category Distribution</h2>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {categories.map((c) => {
              ...
              return (
                <div key={c.name} className="p-4 rounded-xl border border-slate-200 bg-white/40 flex flex-col justify-between h-24 hover:border-slate-300 hover:bg-white transition-all duration-300">
                  ...
                  <div>
                    <div className="flex items-baseline gap-1.5">
                      <span className="text-2xl font-black text-slate-850">{c.count}</span>
        ...
        <div className="glass-panel rounded-2xl p-6 space-y-6 flex flex-col justify-between">
          <div>
            <h2 className="text-lg font-black text-slate-950 pb-3 border-b border-slate-200/80 mb-6">
              Quick Operations
            </h2>
            <div className="flex flex-col gap-4">
              <Link
                href="/pois/new"
                className="flex flex-col p-4 rounded-xl border border-slate-200 bg-white/40 hover:bg-teal-50/[0.3] hover:border-teal-200/80 transition-all group duration-300 relative shadow-2xs"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-lg bg-teal-50 text-teal-600 border border-teal-100 group-hover:bg-teal-600 group-hover:text-white transition-colors duration-300">
              ...
              <Link
                href="/pois/import"
                className="flex flex-col p-4 rounded-xl border border-slate-200 bg-white/40 hover:bg-indigo-50/[0.3] hover:border-indigo-200/80 transition-all group duration-300 relative shadow-2xs"
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-lg bg-indigo-50 text-indigo-600 border border-indigo-100 group-hover:bg-indigo-600 group-hover:text-white transition-colors duration-300">
              ...
              <Link
                href="/pois"
                className="flex flex-col p-4 rounded-xl border border-slate-200 bg-white/40 hover:bg-slate-50 hover:border-slate-300 transition-all group duration-300 relative shadow-2xs"
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-extrabold text-sm text-slate-800 group-hover:text-slate-950 transition-colors">Manage Coordinates</span>
                </div>
